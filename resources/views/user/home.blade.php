@extends('layouts.user.app')

@section('title', 'Dashboard')

@section('content')
	<!-- Hero Section -->
	<section class="py-5 bg-light text-center">
		<div class="container">
			<h1 class="fw-bold mb-3">
				Welcome to <span class="text-primary">Baby Shop</span>
			</h1>
			<p class="lead text-muted">
				Everything Baby Needs, One Happy Place
			</p>
			<a href="#produk" class="btn btn-primary btn-lg mt-3">Shop Now!</a>
		</div>
	</section>

	<!-- Products Section -->
	<div class="container">
		<section id="produk" class="py-5">
			<div class="container">
				<h2 class="text-center mb-5 fw-bold">Featured Products</h2>
				<div class="row g-5 mt-3">
					@forelse($products as $product)
						<div class="col-md-3">
							<div class="card h-100 shadow-sm">
								<img src="{{ asset('storage/' . $product->foto) }}" class="card-img-top" alt="{{ $product->nama }}">
								<div class="card-body text-center">
									<h5 class="card-title">{{ $product->nama }}</h5>
									<p class="card-text text-muted">Rp {{ number_format($product->harga, 0, ',', '.') }}</p>
									<a href="{{ route('products.show', $product->id) }}" class="btn btn-outline-primary">Product Details</a>
								</div>
							</div>
						</div>
					@empty
						<p class="text-center">No products available yet.</p>
					@endforelse
				</div>
			</div>
		</section>
	</div>
@endsection
